...
class TestPose:
    ...
    def test_rotation_matrix_90deg_z(self) -> None:
        ...
        pose = Pose(
            position=np.array([0.0, 0.0, 0.0]), quaternion=np.array([w, 0.0, 0.0, z])
        )
    ...
    def test_from_matrix(self) -> None:
        ...
        assert (
            abs(abs(pose.quaternion[0]) - 1.0) < 1e-5
            or abs(np.linalg.norm(pose.quaternion[1:]) - 0.0) < 1e-5
        )
    ...
    def test_from_euler(self) -> None:
        ...
        np.testing.assert_array_almost_equal(
            pose.rotation_matrix, expected_R, decimal=5
        )
    ...
    def test_inverse(self) -> None:
        ...
        pose = Pose.from_euler(
            np.array([1.0, 0.0, 0.0]), np.array([0.0, 0.0, math.pi / 2]), seq="xyz"
        )
        ...
        np.testing.assert_array_almost_equal(
            result.rotation_matrix, np.eye(3), decimal=5
        )
    ...
    def test_rotation_property(self) -> None:
        ...
        pose = Pose.from_euler(np.zeros(3), np.array([0.1, 0.2, 0.3]), seq="xyz")
...
class TestTwist:
    ...
    def test_twist_creation(self) -> None:
        ...
        twist = Twist(
            linear=np.array([1.0, 2.0, 3.0]), angular=np.array([0.1, 0.2, 0.3])
        )
    ...
    def test_as_vector(self) -> None:
        ...
        twist = Twist(
            linear=np.array([1.0, 2.0, 3.0]), angular=np.array([4.0, 5.0, 6.0])
        )
...
class TestWrench:
    ...
    def test_wrench_creation(self) -> None:
        ...
        wrench = Wrench(
            force=np.array([10.0, 20.0, 30.0]), torque=np.array([1.0, 2.0, 3.0])
        )
    ...
    def test_as_vector(self) -> None:
        """Test conversion to 6D vector."""
        wrench = Wrench(
            force=np.array([1.0, 2.0, 3.0]), torque=np.array([4.0, 5.0, 6.0])
        )
...
class TestJointState:
    ...
    def test_partial_state(self) -> None:
        ...
        state = JointState(timestamp=1.0, position=np.array([0.1, 0.2, 0.3]))
...
class TestPseudoKinematicsModel:
    ...
    def test_forward_kinematics_various_configs(
        self, model: PseudoKinematicsModel
    ) -> None:
    ...
    def test_inverse_kinematics_reachable(self, model: PseudoKinematicsModel) -> None:
        ...
        q_solution, success = model.inverse_kinematics(
            target_pose, q_init=np.zeros(6), max_iterations=200
        )
    ...
    def test_target_tracking_convergence(self, robot: PseudoRobotArm) -> None:
        ...
        for _ in range(1000):
            robot.step_target_tracking(
                position_target=target, kp=100.0, kd=20.0, add_gravity_compensation=True
            )
...
class TestControlCommands:
    ...
    def test_free_drive_command(self) -> None:
        ...
        cmd = FreeDriveCommand(
            external_wrench=Wrench.zero(), gravity_compensation_scale=0.8
        )
