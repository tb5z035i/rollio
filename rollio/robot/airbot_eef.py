...
class _AIRBOTStandaloneEEFCommon:
    ...
    def enable(self) -> bool:
        ...
        try:
            ...
            return False
        self._is_enabled = self._sdk_mutator_succeeded(result)
        return self._is_enabled

    def disable(self) -> None:
        if self._eef is None or not self._is_enabled:
            return
        self._eef.disable()
        self._is_enabled = False
        self._control_mode = ControlMode.DISABLED
...
class AIRBOTE2B(_AIRBOTStandaloneEEFCommon, RobotArm):
    ...
    def set_control_mode(self, mode: ControlMode) -> bool:
        ...
        if mode != ControlMode.FREE_DRIVE:
            return False
        if not self._configure_feedback_mode():
            return False
        self._control_mode = mode
        return True
...
class AIRBOTG2(_AIRBOTStandaloneEEFCommon, RobotArm):
    ...
    def _identify_target_position(self, now: float | None = None) -> np.ndarray:
        ...
        midpoint = 0.5 * (lo + hi)
        amplitude = 0.5 * (hi - lo)
        target = midpoint + amplitude * np.sin(2.0 * np.pi * self.IDENTIFY_OSCILLATION_HZ * t)
