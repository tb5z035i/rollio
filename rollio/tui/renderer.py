...
def render_frame(bgr: np.ndarray, w: int, h: int, mode: str = "256") -> bytes:
    ...
    if builder is None:
        raise ValueError(f"Unknown render mode {mode!r}; choose from {RENDER_MODES}")
...
def render_depth(
    ...
    if frame.dtype == np.uint16:
        ...
        normalized = np.clip(
            (frame.astype(np.float32) - min_val) / max(1, max_val - min_val) * 255,
            0,
            255,
        ).astype(np.uint8)
    ...
    if mode == "turbo":
        ...
        cmap_idx = (
            normalized.astype(np.float32) * (len(_TURBO_RED_BLUE) - 1) / 255
        ).astype(np.uint8)
...
def normalize_depth_for_display(
    depth: np.ndarray, min_mm: float = 200, max_mm: float = 5000
) -> np.ndarray:
    ...
    """
    return np.clip(
        (depth.astype(np.float32) - min_mm) / max(1, max_mm - min_mm) * 255, 0, 255
    ).astype(np.uint8)


def calc_render_size(
    cam_w: int, cam_h: int, avail_w: int, avail_h: int, max_cols: int = 120
) -> tuple[int, int]:
