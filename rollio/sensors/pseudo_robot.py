...
class PseudoRobot(RobotSensor):
    ...
    def __init__(self, name: str = "pseudo_arm",
                 n_joints: int = 6, role: str = "follower") -> None:
        self._name = name
        self._n = n_joints
        self._role = role
        self._open = False
    ...
    def read(self) -> tuple[float, dict[str, np.ndarray]]:
        ...
        vel = self._amp * 2 * math.pi * self._freq * np.cos(
            2 * math.pi * self._freq * t + self._phase)
    ...
    def info(self) -> SensorInfo:
        return SensorInfo(
            name=self._name, sensor_type="robot",
            properties={"num_joints": self._n, "type": "pseudo",
                        "role": self._role})
